...
class CreditController extends Controller
{
    ...
    public function applyto()
    {
        ...
        if($re==0)
        {
            echo "<script>alert('请先实名认证');location.href='/personal/personal'</script>";
        }
        else
        {
            $data=Input::all();
            ...
            //应还款期数
            $back['periods']=$data['loan_long'];

            //本金
            $back['principal']=$data['loan_money'];
       ...
       if( $usermoney< $num)
       {
           ...
           if($update)
           {
               ...
               if($nm<$num)
               {
                   ...
                   if($up)
                   {
                       $upd=DB::table('loan')->decrement('loan_long', 1, ['user_id' => $user_id]);
                       if($upd)
                       {
                           $ree=DB::table('back')->where('user_id','=',$user_id)->get();
                           foreach($ree as $k=>$v)
                           {
                           ...
                           $ab=ceil($principal/$periods);
                           $upda=DB::table('back')->decrement('principal', $ab, ['user_id' => $user_id]);
                           if($upda)
                           {
                               return 1;
                           }
